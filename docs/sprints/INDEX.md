...
| **027** | **AI Guidance Orchestrator (deterministic plan peste Compliance Gate + Preventive Engine + Findings + Coverage Matrix; role-aware guidance pentru IMM / AI Builder / Cabinet; omitted actions + plan diff + after-action regeneration + Audit Pack ai-guidance/; provider-grade gaps incl. Art. 10/13/15/23-25/47/86/GPAI documented; NO auto-execution, NO invented law, DS port deferred)** | ✅ DONE (local) | uncommitted | manual: Codex | [sprint-027-ai-guidance-orchestrator.md](sprint-027-ai-guidance-orchestrator.md) |
| **028** | **Light DS + AI Guidance UX Polish (Claude Design direction applied production-safe: light-first runtime tokens + dark fallback + Tailwind v4 semantic bridge; shell brand/sidebar/workspace ribbon; dashboard wide layout; GuidancePlanPanel work-plan hierarchy + focused drawer + owner/effort + omissions/diff; inline AI Guidance card in Resolve expanded finding; NO role/workspace change, NO legal verdict drift, raw `_design-claude/` stays reference only)** | ✅ DONE (local) | uncommitted | manual: Codex | [sprint-028-light-ds-ai-guidance-polish.md](sprint-028-light-ds-ai-guidance-polish.md) |
| **029** | **AI Monitoring & Evidence Foundation (ROADMAP LOCKED — de implementat dupa DS + QA E2E: port selectiv Repo/Config Drift Monitor din CompliScan; `ai-compliance.yaml`; Monitoring Maturity per AI system; drift -> finding -> AI Guidance trigger; Audit Pack monitoring/repo-drift/runtime-logging sections. Scope: compliance evidence layer peste MLOps/logging existent, NU full MLOps platform.)** | ⏳ PLANNED | — | manual: Codex | [monitoring evidence mandate](../strategic/compliroai-ai-monitoring-evidence-layer-2026-05-25.md) |
