/**
 * Sprint 019 — PMM Plan markdown / PDF export.
 *
 * GET /api/pmm/:id/export             → text/markdown
 * GET /api/pmm/:id/export?format=pdf  → application/pdf (cu branding white-label)
 */
...
export async function GET(
  ...
  try {
    ...
    const markdown = buildPmmMarkdown(record, ctx.orgName ?? "", systemName)

    const fileStem = `pmm-${slug(ctx.orgName ?? "org")}-${slug(record.title)}-${new Date()
      .toISOString()
      .slice(0, 10)}`
    ...
    return NextResponse.json(
      { error: "Nu am putut exporta planul PMM." },
      { status: 500 },
    )
  }
}
