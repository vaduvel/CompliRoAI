/**
 * Sprint 017 — Oversight Protocol markdown / PDF export.
 *
 * GET /api/oversight/:id/export             → text/markdown
 * GET /api/oversight/:id/export?format=pdf  → application/pdf (cu branding white-label)
 */
...
export async function GET(
  ...
  try {
    ...
    const markdown = buildOversightMarkdown(record, ctx.orgName ?? "", systemName)

    const fileStem = `oversight-${slug(ctx.orgName ?? "org")}-${slug(record.title)}-${new Date()
      .toISOString()
      .slice(0, 10)}`
    ...
    if (format === "pdf") {
      ...
      const pdf = await generatePdfFromMarkdown(markdown, {
        orgName: ctx.orgName ?? "",
        title: `Oversight Protocol — ${record.title}`,
        branding,
    ...
    return NextResponse.json(
      { error: "Nu am putut exporta protocolul Oversight." },
      { status: 500 },
    )
  }
}
